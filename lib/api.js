...
export async function getTasks() {
  return await _ds_post('list', {additional: 'transfer'});
}
...
export async function pauseTask(id) {
  return await _ds_post('pause', {id});
}

export async function resumeTask(id) {
  return await _ds_post('resume', {id});
}

export async function deleteTask(id) {
  return await _ds_post('delete', {id});
}
...
async function _ds_post(method, params = {}) {
  const {host, sid} = await STORAGE.get();
  if (sid === "") {
    return {success: false, message: 'Auth required.'};
  }
  return await _post(`${host}${API_PATH_DS}`, {
    api: 'SYNO.DownloadStation.Task', version: 3, _sid: sid, method, ...params
  });
}

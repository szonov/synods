...
function iconCheck() {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M5 12l5 5l10 -10" /></svg>`;
}
...
class Task {
  ...
  get isPauseVisible() {
    return ["downloading", "hash_checking", "waiting", "seeding"].includes(this.status);
  }

  get isResumeVisible() {
    return ["paused", "finished", "error"].includes(this.status);
  }
  ...
  _renderStatusLine($div) {
    ...
    switch (this.statusType) {
      ...
      case "other":
        icon = this.status === "paused" ? iconPause() : iconClock();
...
export class TaskList {
  ...
  render(synoTaskList) {
    ...
    synoTaskList.forEach((synoTask) => {
      ...
      hasTasks = true;
      hasFinishedTasks = hasFinishedTasks || (task.status === "finished")
    ...
    return { speedDownload, speedUpload, hasTasks, hasFinishedTasks };
  }
